...
class Artikel_model extends Model
{
    ...
    public function list_editorial()
    {
        $query = DB::table('artikel')
             ->join('kategori', 'kategori.id_kategori', '=', 'artikel.id_kategori','LEFT')
            ->join('users', 'users.id_user', '=', 'artikel.id_user','LEFT')
            ->select('artikel.*', 'kategori.slug_kategori', 'kategori.nama_kategori','users.nama')
            ->where(array('artikel.status_artikel'=>'Publish','artikel.jenis_artikel' => 'Editorial'))
            ->orderBy('id_artikel','DESC')
            ->paginate(8);
        return $query;
    }

    public function list_rilis()
    {
        $query = DB::table('artikel')
             ->join('kategori', 'kategori.id_kategori', '=', 'artikel.id_kategori','LEFT')
            ->join('users', 'users.id_user', '=', 'artikel.id_user','LEFT')
            ->select('artikel.*', 'kategori.slug_kategori', 'kategori.nama_kategori','users.nama')
            ->where(array('artikel.status_artikel'=>'Publish','artikel.jenis_artikel' => 'Rilis'))
            ->orderBy('id_artikel','DESC')
            ->paginate(8);
        return $query;
    }
